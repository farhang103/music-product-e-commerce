...
import {
    ...
    RootStateProduct,
    DataProps,
} from "@/types/ProductDetailsProps";
...
const ProductDetail = () => {
    ...
    const { name } = useSelector(
        (rootReducer: RootStateProduct) => rootReducer.productReducer
    );

    useEffect(() => {
        name
            ? null
            : dispatch(findProduct(localStorage.getItem("name_product")));
    }, []);
    ...
    let dataFormated: DataProps[] = data.filter(
        (product: any) => product.slug === name
    );
    ...
    return (
        <>
            <Head>
                <title>
                    audiophile - {dataFormated.map((product) => product.name)}
                </title>
                <meta
                    name="viewport"
